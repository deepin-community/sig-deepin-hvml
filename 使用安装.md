__以下操作请在最新版本的 deepin 中进行！__

## 安装 hvml

````bash
sudo apt update
sudo apt install hvml-purc-bin
````

## 使用说明

1. 请将以下内容保存在`hello.hvml`文件中 
...
要运行此 HVML 程序（请在 hello.hvml 文件所在路径中执行命令），您可以使用`purc`以下方式：

`purc hello.hvml`

 您将看到您的第一个 HVML 程序`Hello, world!` 在终端上打印并退出：

`Hello, world!`

2. 安装hvml-xgui-pro, 请将以下内容保存在`calculator-bc.hvml`工作目录中名为您的第一个 HVML 程序的文件中  
	...
	<hvml target="html" lang="$STR.substr($SYS.locale, 0, 2)">
	    ...
	    <body>
	        ...
	        <div class="container">
	            ...
	            <div class="shadow-none p-3 mb-5 bxguiproxguiproxguiprog-light rounded">
...
```
purc -p purcmc calculator-bc.hvml 
```
...
更多的的示例可以参考[hvml/purc](https://github.com/HVML/PurC)
